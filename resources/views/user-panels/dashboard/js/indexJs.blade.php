<script>
    let rl_id = {!! json_encode(\Auth::user()->role_id) !!}
        ...
        success: function(data) {
            if (data.isSuccess == true) {
                ...
                for(let i = 0 ; i < invoices.length ; i++){
                    ...
                    @if (Request::segment(1) === 'institute')
                        btns = `
    ...
    function submitInvoice(){
        ...
        $.ajax({
            type: 'POST',
            url: "{{route('update_invoice_status')}}",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            data: formData,
