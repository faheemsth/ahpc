            ...
            <tr class="top">
                <td colspan="2">
                    <table>
                        <tr>
                            <td class="title">
                                <img src="{{ optional($invoice->user)->avatar }}" style="width: 100%; max-width: 110px" />
                            </td>
                            <td>
                                Licence No #: {{ $invoice->invoice_id }}<br />
                                <img src="data:image/png;base64,{{ base64_encode(QrCode::format('png')->size(300)->generate($invoice->invoice_id)) }}"
            ...
            <tr class="information">
                <td colspan="2">
                    <table>
                        <tr>
                            <td>
                                <b class="heading">Dear</b> {{ $invoice->user->first_name }}<br />
                                {{ $invoice->user->postel_address }}<br />
                                {{ $invoice->user->email }}<br />
                                <b class="heading">Registered Date</b>
                                {{ Illuminate\Support\Carbon::parse($invoice->user->created_at)->format('d-m-Y') }}
                            </td>
                        </tr>
                        <tr>
                            <td>
